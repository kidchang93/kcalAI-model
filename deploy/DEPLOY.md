...
>
> ⚠️ **`release` 브랜치는 2026-07-12에 멈춰 있다**(master보다 22커밋, main보다 24커밋 뒤). 이 문서가
> 오래 "배포 브랜치는 release"라고 적어 뒀지만 실제 배포는 `local_deploy.sh`가 작업 트리를 올리는
> 방식이라 그 브랜치를 거치지 않는다. **`deploy/redeploy.sh`는 아직 `release`를 pull하므로, 지금
> 그걸 쓰면 두 달 가까이 낡은 코드가 배포된다.** 서버 git pull 방식으로 돌아가려면 `release`를 먼저
> 최신화하거나 `redeploy.sh`의 `BRANCH`를 고칠 것 (5장 각주).
>
...
> **대안(서버 git pull 방식) — 지금 쓰면 낡은 코드가 올라간다**: 서버에 repo를 clone해 두고
> `deploy/redeploy.sh`로 pull·재시작하는 경로가 있으나, 이 스크립트의 `BRANCH="release"`가 가리키는
> `release` 브랜치는 **2026-07-12에 멈춰 있다**(위 헤더 참조). 쓰려면 `release`를 최신화하거나 `BRANCH`를
> `master`로 바꿔야 한다. 로컬 주도 방식(`local_deploy.sh`)을 쓰면 서버에 git/원격 접근이 필요 없다 —
> **현재 운영은 이 방식이다.**
