...
#
# - 나트륨: KDIGO 2024 가 CKD 전반에 1일 2 g 미만(소금 5 g)을 권고한다(2C). 투석은 KSN2 기준
#   3,000 mg 으로 완화된다. 병기가 정해지면 단일 값이 나오므로 **상한 대비 게이지를 그린다.**
...
# 병기를 모를 때의 안내. 상한을 임의로 고르지 않고 범위를 알린다 — 비투석에 3,000을 대면
# 느슨하고, 투석에 2,000을 대면 과잉 제한이다(§4의 "판단을 남긴다"와 같은 태도).
SODIUM_STAGE_UNKNOWN_NOTE = (
    "신장 질환은 투석 여부에 따라 나트륨 목표가 2,000~3,000 mg 으로 달라집니다. "
    "내 정보에서 투석 여부를 입력하면 그 기준으로 알려드릴게요."
)
