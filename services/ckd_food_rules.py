...
- 대한신장학회 「제2권: 혈액투석 환자를 위한 영양-식생활 관리」
  (이하 KSN2) — 단백질 1.2 g/kg, 열량, 나트륨 2,000 mg(p114), 인 800–1,000 mg(p141),
  과일·채소 칼륨 3단계 분류.
...
STAGE_TARGETS: dict[str, dict] = {
    ...
    CKD_STAGE_HEMODIALYSIS: {
        ...
        "sodium_mg_max": 2000,  # KSN2 p114 · KDOQI 2020 6.5.1(CKD 5D, 1C) — 2026-09-16 3,000에서 정정
        ...
        "source": "KSN2 (단백질 1.2 g/kg · 인 p141 · 나트륨 p114)",
    },
    CKD_STAGE_PERITONEAL: {
        "protein_g_per_kg": (1.2, 1.3),  # 증량 (복막으로 단백 손실) — 국가건강정보포털/KDIGO
        "sodium_mg_max": 2000,  # KDOQI 2020 6.5.1 은 CKD 5D(복막투석 포함)를 한 값으로 묶는다
...
# ── 하루 누적 표시의 기준선 (2026-07-23, `docs/CKD_NUTRITION.md` 3-6) ──────────
#
# **나트륨만 '상한'이고 칼륨·인은 '참고치'다.** 이 구분이 이 절의 전부다.
#
# - 나트륨: KDIGO 2024 가 CKD 전반에 1일 2 g 미만(소금 5 g)을 권고하고(2C), KDOQI 2020 권고
#   6.5.1 은 투석(CKD 5D)까지 같은 값으로 묶는다(1C). 병기와 무관한 단일 값이라
#   **상한 대비 게이지를 그린다.**
# - 칼륨·인: KDOQI 2020 은 둘 다 **혈청 수치를 정상 범위로 유지하도록 개인화**하라고 할 뿐
#   하루 mg 상한을 권고하지 않는다. KDIGO 2024 는 한 발 더 나아가 보편적 칼륨 제한에서
#   물러섰다(가공식품의 칼륨은 줄이되 과일·채소는 제한하지 말 것). 따라서 **상한으로 쓰지
#   않는다** — 아래 값은 국내 임상 실무에서 통용되는 참고치일 뿐이고, 게이지가 아니라
...
POTASSIUM_PHOSPHORUS_REFERENCE_NOTICE = (
    "칼륨·인은 하루 권장량이 정해져 있지 않고 혈액검사 수치에 따라 개인별로 달라집니다"
    "(KDOQI 2020). 표시되는 참고치는 투석 환자에게 흔히 쓰이는 값이며 진료 기준이 아닙니다."
)

# 병기를 모를 때의 안내. **2026-09-16부터 나트륨은 병기와 무관하게 2,000 mg 이다** — 예전에는
# 투석이 3,000 이라 병기 없이는 상한을 고를 수 없었고, 이 문구가 그 범위를 알렸다. 이제 갈리는
# 것은 칼륨·인 참고치와 과일 분류뿐이라 안내도 그쪽을 가리킨다.
SODIUM_STAGE_UNKNOWN_NOTE = (
    "투석 여부를 입력하면 칼륨·인 참고치와 과일 분류를 그 기준으로 알려드릴게요. "
    "나트륨 하루 2,000 mg 은 투석 여부와 관계없이 같습니다."
)
...
#
# **투석 3,000 mg 은 2026-09-16 에 2,000 으로 정정했다.** 2026-09-13 에는 KSN2 안의 충돌을
# (p96 "3,000 mg/day" vs p114 "2,000 mg 이하")를 출처에 드러내고 값은 3,000 을 두었는데, 그것은
# 충돌을 노출한 것이 아니라 **둘 중 느슨한 쪽을 이미 고른 것**이었다. 더 상위·최신 근거를 찾아
# 정했다 (원문 확인):
#
# - **KDOQI 2020 권고 6.5.1**(AJKD 76(3) Suppl 1): "In adults with CKD 3-5 (1B), **CKD 5D (1C)**,
#   or posttransplantation (1C), we recommend limiting sodium intake to less than 100 mmol/d
#   (or <2.3 g/d)". CKD 5D 가 투석이다 — **투석에 완화를 주지 않는다.**
# - **KDIGO 2024**: CKD 전반 하루 2 g 미만(소금 5 g).
# - **KSN2 p114**(염분 전용 섹션, Key Message 있음): "혈액투석을 하는 경우에도 하루에 허용되는
#   염분 섭취량은 5 g(나트륨으로는 2,000 mg) 이하", "투석을 하지 않는 만성 콩팥병 환자와 동일하게".
# - 질병관리청 국가건강정보포털·국내 상급종합병원 영양팀 자료도 투석 2,000 mg.
#
# 반대편 근거는 KSN2 p96 하나인데, 그것은 **수분 관리 장의 실천 팁 목록 괄호 안** 숫자이고
# 2차 인용(임상영양관리지침서 제3판)이다. p114 는 나트륨 전용 섹션의 본문 권고다 — 위계가 다르다.
# 더 엄격한 2,000 을 택한 것은 KDIGO 2024·국내 자료와 같고, KDOQI 2.3 g 보다도 안쪽이다.
SODIUM_LIMIT_CITATIONS: dict[str, str] = {
    CKD_STAGE_NONDIALYSIS: "대한신장학회 영양-식생활 관리 1권 p101·105",
    CKD_STAGE_HEMODIALYSIS: "대한신장학회 영양-식생활 관리 2권 p114 · KDOQI 2020 권고 6.5.1",
    CKD_STAGE_PERITONEAL: "KDOQI 2020 권고 6.5.1 (CKD 5D) · KDIGO 2024",
}


# 병기와 무관한 CKD 나트륨 상한. 세 병기가 같은 값이 된 뒤로(2026-09-16) 병기를 몰라도 줄 수 있다.
CKD_SODIUM_MG_MAX = 2000


def sodium_daily_limit_mg(stage: str | None) -> int:
    """CKD 의 나트륨 1일 상한. **병기를 몰라도 값이 나온다** (2026-09-16).

    예전에는 투석이 3,000 이라 병기 미상이면 None 이었고, 투석 여부를 입력하지 않은 CKD
    사용자는 하루 누적에 게이지가 없었다 — 가장 중요한 축이 비어 있었던 셈이다. 세 병기가 같은
    2,000 이 된 지금은 고를 것이 없으므로 상한을 준다(근거는 SODIUM_LIMIT_CITATIONS 주석).
    """
    targets = STAGE_TARGETS.get(stage or "")

    return int(targets["sodium_mg_max"]) if targets is not None else CKD_SODIUM_MG_MAX
