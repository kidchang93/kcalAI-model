...
### `GET /api/me/trends` — 질환 축 기간 추이 (2026-07-25 추가)

응답에 **`nutrients`**가 추가됐다(하위호환, 해당 질환이 없으면 null). 리포트가 kcal 그래프만 보여주면 **정작 이 앱의 대상 사용자에게 중요한 숫자가 어디에도 없다** — 만성질환 관리에서 하루는 흔들리고 추세가 말을 한다. 목표 지표의 "검사 수치가 나빠졌을 때 되짚을 수 있는가"(`PRODUCT_STRATEGY.md` §0-2)가 이 화면을 요구한다.

축 구조는 하루 누적(28장)과 같은 규칙이다 — 나트륨만 `limit_mg`, 칼륨·인은 `reference_mg`. 여기에 기간 전용 필드가 붙는다:
...
`meal_items`에 **`sodium_mg`·`potassium_mg`·`phosphorus_mg`·`sugar_g`**(전부 nullable)를 더했다. 값은 **먹은 양 기준**이다 — 1인분 실측 × `serving_ratio`. 실측이 없는 음식은 NULL이고, 그 사실은 하루 누적의 `measured_items`로 이미 드러난다.

**왜**: 그전까지 하루 누적(28장)과 경고(16장)는 저장된 `food_label`로 `food_nutrition`을 **매번 다시 조회**해 수치를 만들었다. 그래서 DB 값이 바뀌면 **과거 기록이 말하는 수치도 소급해 바뀐다.** 2026-07-25에 실제로 일어났다 — 1인분 기준을 4,536행 고치고(홀케이크·피자) 동명 행 선택 규칙도 바꾸자(라면 226.6 g → 550 g) 사용자의 지난 기록이 말하는 나트륨이 조용히 달라졌다. **기록이 아니라 추정이었던 것이다.**
...
- **`MealItemResponse`에 4필드가 실린다 (2026-08-03 추가)** — `sodium_mg`·`potassium_mg`·`phosphorus_mg`·`sugar_g`, 전부 `float|null`. 기존 필드는 불변이고 앱은 없으면 `null`로 읽는다(옛 서버 호환).
  ...
  >
  > **등급(`tier`)은 담지 않는다.** 수치는 사실이라 굳히지만 등급은 해석이라 조회 시점 규칙으로 다시 판정한다. 굳히면 규칙을 고쳐도 과거 기록만 낡은 판정을 달고 남는다.
  >
  > 회귀는 `tests/test_meal_nutrition_snapshot.py`가 건다 — **`_SNAPSHOT_COLUMNS`를 진실로 삼아 대조**하므로 축을 추가하면서 응답 필드를 빠뜨리면 먼저 깨진다. 이 실수는 에러 없이 조용히 실패하는 종류라 테스트가 유일한 방어선이다.
- 기존 행은 마이그레이션이 **현재 DB 값으로 백필**한다(기록 당시 값이 아니라는 한계는 있으나, 비워 두면 조회 경로가 영구히 폴백을 안는다). 조회 규약은 경고와 같은 **정확·공백무시 일치**다.
- 회귀는 `tests/test_day_nutrition.py::TestSnapshotIsImmutable`이 건다 — 나중에 DB를 고쳐도 과거 합계가 그대로이고, **새 기록은 고쳐진 값을 쓴다**(동결이지 무시가 아니다).

### `GET /api/meals?date=D` — 정렬은 `logged_at` → `id` (2026-07-25 확정)

시각 하나로는 순서가 정해지지 않는다. **같은 날 기록은 `logged_at`이 같은 값으로 몰리기 때문**이다 — 앱이 과거 날짜를 UTC 정오로 앵커한다(아래). 시각만으로 정렬하면 나머지 순서를 DB 물리 순서가 결정하고, 항목을 더해 UPDATE된 끼니가 목록 맨 뒤로 밀린다. 사용자에겐 **"방금 추가한 기록이 안 보인다"**로 읽힌다(2026-07-25 재현). `id`를 동점 처리 키로 두어 만든 순서를 유지한다 (`tests/test_meal_ordering.py`).

**`logged_at` 앵커 규약**: 과거 날짜에 남기는 기록만 **UTC 정오**로 앵커한다(어느 타임존에서 저장해도 UTC 날짜가 D로 고정 → 캘린더 셀 D에서 다시 보인다). **오늘 기록은 지금 시각**을 쓴다 — 앱이 오늘까지 정오로 앵커하면 아침에 먹은 것도 21:00(KST)으로 표시되고, 하루의 끼니가 전부 같은 시각이 되어 시각 정렬이 죽는다. 단 지금 시각의 UTC 날짜가 D와 어긋나는 시간대(KST 새벽)에서는 앵커로 되돌아간다 — 표시 시각의 정확도보다 **그 날짜 칸에서 사라지지 않는 것**이 우선이다 (`k-calAI-RN`의 `dayAnchorLoggedAt`).
...
`sensitive_health` 철회 시 아래를 **물리 삭제**한다(파기). 동의 행 자체는 `revoked_at`을 채워 증빙으로 남긴다. 재동의하면 새 동의 행을 만들고(이력 보존) 데이터는 처음부터 다시 입력받는다.
...
> 403으로 막혔지만 파기는 "접근을 막는 것"이 아니라 "없애는 것"이다. 탈퇴 연쇄
> (`account_service.delete_account`)와 **서로 다른 목록**이라 한쪽만 고치면 반복된다.
> 규약은 `tests/test_consent_revoke_destroys.py`가 고정한다.
>
> **앱의 철회 확인 문구도 같은 작업 단위에서 고친다** (`app/me/consents.tsx`) — 무엇을 잃는지
> 모르고 누르면 고지의 의미가 없다.
...
> **개정 (2026-07-25) — 당류 축과 단위.** 경고 축(`nutrient`)에 **`sugar`**가 추가됐다(당뇨, 리비전 0024). 다른 축과 두 가지가 다르다:
>
> 1. **등급(`tier`)이 없다.** 간식·음료 행의 "1인분"이 제품 한 통·한 판이라 경계를 대면 거의 전부 '높음'이 된다 (`CHRONIC_NUTRITION_SOURCES.md` §2-5의 실측). 발동은 **이름 축만** 쓴다 — 지침 대상이 총당류가 아니라 첨가당이라, 실측으로 발동하면 지침이 유익하다고 명시한 생과일·흰우유가 걸린다(§2-2).
> 2. **단위가 g다.** 그래서 응답에 **`nutrient_unit`**(`"mg" | "g" | null`)이 추가됐다. 필드명은 `nutrient_mg`로 두고 단위를 따로 내린다 — 계약을 깨지 않기 위해서다. 옛 앱은 이 필드를 무시하고 mg로 읽으므로 하위호환이며, 1인분으로 볼 수 없는 당류(50 g 초과)는 `nutrient_mg`가 **null**로 내려간다(수치 없이 경고만).
>
> **개정 (2026-07-25) — 판정하지 못한 음식.** 응답에 **`unmeasured: string[]`**가 추가됐다(하위호환, 옛 앱은 무시). 실측이 없고 지침 키워드에도 없으면 경고가 한 건도 나가지 않는데, 화면에서는 그것이 **"괜찮다"로 읽힌다** — 신장병 환자가 돈까스·보쌈·통닭(전부 curated 결측 행이다)을 기록하면 아무 말도 나가지 않았다. 근거가 없다는 사실 자체가 사용자가 알아야 할 근거다(`PRODUCT_STRATEGY.md` §0-1). 판정 대상은 **실측이 필요한 축**(나트륨·칼륨·인)뿐이고 당류는 이름 축만 쓰므로 여기 들어가지 않으며, 질환이 없는 사용자에게는 빈 배열이다. 경고가 나간 음식은 판정된 것이므로 제외한다.

> 같은 리비전에서 **당뇨에 `low_sodium` 태그**를 넣어 나트륨 축을 켰다(KDA2025 권고 9, 2,300 mg). 그전까지 당뇨 사용자는 나트륨 경고를 전혀 받지 못했다. ⚠️ 질병에 영양소 축이 붙으면 판정이 `exclude_keywords` 경로를 더 이상 타지 않으므로, 그 키워드를 축이 **흡수**해야 한다 — 빠뜨리면 기존 경고가 조용히 사라진다(`tests/test_record_warnings_ckd.py`가 회귀를 건다).
...
1. ~~**가입 시 유료 `plan_code` 선택이 결제 없이 그대로 부여된다** (`POST /api/auth/kakao/signup`). `current_period_end`가 null이라 만료 강등도 비켜간다 — 즉 **무료로 Premium을 얻는 경로가 남아 있다.**~~ **해소** — `subscription_service.create_subscription`이 `plan.price_krw > 0`이면 lite로 강등한다(유료 선택은 '의사표시'로만 받고, 400으로 막지 않는 이유는 가입 자체가 실패하면 서비스에 들어오지도 못하기 때문). 유료 부여 경로는 `POST /api/billing/confirm` 하나뿐이다.
2. ~~**웹훅 미구현.** 토스 결제 상태 변경(취소·매입 실패)을 서버가 알 수 없다. 현재는 우리가 부른 청구의 응답만 원장에 반영한다.~~ **해소** (2026-07-26, **29장**) — `POST /api/billing/webhook`. 단 토스 상점관리자에 URL을 등록해야 동작한다.
...
## 28. 하루 질환 축 누적 — 홈의 "오늘의 영양" (2026-07-23 확정 — 리비전 0023)

### 왜

경고(16장)는 **기록 확정 직전 그 순간**에만 보인다. 저장하면 사라져서, 만성질환자가 "오늘 나트륨 얼마나 먹었지"를 볼 화면이 없었다. 홈은 칼로리 링과 끼니 카드뿐이라 일반 다이어트 앱과 구조가 같았다 — 정작 대상 사용자에게 중요한 숫자가 홈에 없다는 뜻이다 (`PRODUCT_STRATEGY.md` §1).

### 계약 — `GET /api/me/summary` += `nutrients`

```jsonc
"nutrients": {                    // 해당 질환이 없으면 null (앱은 카드를 그리지 않는다)
  "axes": [
    {
      "nutrient": "sodium",       // sodium | potassium | phosphorus
      "label": "나트륨",
      "consumed_mg": 1200.5,
      "limit_mg": 2000,           // **나트륨에만 있다.** null 이면 게이지 없이 수치만
      "reference_mg": null,       // 상한이 아닌 실무 참고치(투석 칼륨·인). 게이지로 쓰지 않는다
      "basis": "고혈압 기준 하루 2,000 mg · 대한고혈압학회 진료지침 2026 권고 21",
      "measured_items": 2         // 이 축의 실측을 찾은 항목 수
    }
  ],
  "total_items": 2,
  "notice": "…"                   // 추정치라는 사실 + (참고치를 노출하면) 지침 권고가 아니라는 고지
}
```
...
**`basis`에는 출처가 같은 줄에 붙는다** (2026-09-13, KCAL-15) — 앱·리포트는 이 문자열을 그대로 쓰므로 출처를 따로 조립하지 않는다. 문자열 형식이 바뀌는 것은 계약 변경이 아니다(자유 텍스트).

**② 병기를 모르면 나트륨 상한도 만들지 않는다.** 비투석 2,000과 투석 3,000 중 하나를 임의로 고르면 한쪽에게는 반드시 틀린 기준이다(투석 환자에겐 과잉 제한, 비투석 환자에겐 느슨). 대신 `basis`에 범위를 알리고 병기 입력을 유도한다. 여러 질환이 겹치면 **더 엄격한 쪽**을 쓴다(투석 3,000 + 고혈압 2,000 → 2,000).

**③ 못 센 항목 수를 숨기지 않는다.** 실측이 없는 음식은 합계에서 빠져 실제보다 적게 보인다. `measured_items` < `total_items`면 앱이 "기록 N개 중 M개 음식의 실측만 반영됐어요"라고 밝힌다. 감추면 커버리지 구멍이 "적게 먹었다"로 읽힌다 — `PRODUCT_STRATEGY.md` §5-2의 curated 결측 사건과 같은 종류의 실수다.

### 구현

- `services/day_nutrition.py`. 실측 조회는 **경고와 같은 규약**(`nutrition_service.measured_nutrition_for` — 정확·공백무시 일치만)을 쓴다. 두 곳이 다르게 찾으면 "경고는 떴는데 합계에는 안 잡히는" 음식이 생긴다.
- 영양소는 `meal_items`에 저장하지 않는다(kcal만). 조회 때 `food_label`로 실측 행을 찾아 `serving_ratio`를 곱한다 — DB 값이 보정되면 과거 합계도 따라 바뀌지만, 이 수치는 참고용 안내라 그 편이 낫다고 판단했다.
- 하루 경계는 끼니 조회와 같은 **UTC 자정**이다.
- 당뇨는 `dietary_tags`에 나트륨이 없지만 1일 상한이 지침에 있어(KDA2025 권고 9 — 2,300 mg) 축을 준다. 태그가 아니라 근거를 따른다.
- 테스트: `tests/test_day_nutrition.py` 11건.

### 리비전 0023 — `user_health_profiles.ckd_stage`

`nondialysis` / `hemodialysis` / `peritoneal`, 미입력은 NULL. **암호화하지 않는다** — 조회 시 기준선을 고르는 기능 키라 condition·allergen을 평문으로 둔 판단과 같다(0013의 범위 정정 이력).

- `GET·PUT /api/me/health-profile` += `ckd_stage`. ⚠️ 이 PUT은 **전체 교체**라 병기만 바꿀 때도 혈액형·Rh를 함께 보내야 한다.
- `GET /api/meta/options` += `ckd_stages` (참조 테이블이 아니라 지침에서 온 고정 3종이지만, 라벨은 서버가 정한다 — 앱이 의학 용어를 임의로 쓰지 않게).
- **단백질 개인 목표(g/kg) 산출은 여전히 하지 않는다** (`CKD_NUTRITION.md` §4 유지). 병기는 나트륨 기준선과 칼륨·인 참고치 노출에만 쓴다.

---

## 29. 토스 웹훅 — 바깥에서 일어난 상태 변화 동기화 (2026-07-26 확정 — 신규 테이블 없음)

### 왜

24장까지의 원장은 **우리가 부른 청구의 응답만** 반영했다. 그래서 두 가지가 새어 나갔다.

1. **상점관리자에서 손으로 취소한 결제가 원장에 남지 않는다.** 환불 규정을 약관에 쓰려면 이행 수단과 **기록**이 함께 있어야 하는데(`LEGAL_COMPLIANCE.md` §2), "환불했다"를 증명할 자리가 비어 있었다. 그래서 지금까지는 "환불은 반드시 `scripts/refund_payment.py`로"라는 절차적 제약을 걸어 두고 있었다.
2. **돈은 나갔는데 기록은 실패인 상태.** 청구 HTTP 응답을 받기 전에 타임아웃·프로세스 종료가 나면 토스에는 승인이, 우리에겐 `failed`가 남는다. 사용자는 결제하고도 무료 회원이다.

웹훅은 이 둘을 **바깥에서 알려 주는 유일한 경로**다.

### 검증 — 본문에는 서명이 없다

토스의 `tosspayments-webhook-signature` 헤더는 **지급대행(`payout.changed`)·셀러(`seller.changed`) 이벤트에만** 붙는다(공식 문서 「웹훅 이벤트」). 결제 웹훅(`PAYMENT_STATUS_CHANGED`·`CANCEL_STATUS_CHANGED`)에는 서명이 없다. 즉 **본문만 보고 원장을 고치면 누구나 "그 결제 취소됐다"고 우리 서버에 알릴 수 있다.**

그래서 규약을 뒤집었다 — **웹훅은 알림일 뿐이고, 사실은 우리가 확인한다.**

| 무엇을 | 어디서 |
|---|---|
| `orderId` | 웹훅 본문 (이것만 읽는다) |
| 상태·금액·승인시각·취소사유 | **`GET /v1/payments/orders/{orderId}`** — 우리 시크릿 키로 인증한 조회 |

`paymentKey`가 아니라 `orderId`로 조회하는 이유: `orderId`는 **우리가 만들어 원장에 갖고 있는 값**이라, 외부에서 받은 식별자를 믿지 않고도 조회할 수 있다.

**모르는 주문이면 토스를 부르지 않는다.** 원장에 없는 `order_id`는 조회 없이 즉시 무시한다 — 이게 없으면 임의의 주문번호를 던지는 것만으로 우리가 결제사 API를 대신 두드리게 만들 수 있다(증폭).

### 계약 — 상태코드가 곧 재전송 정책이다

```
POST /api/billing/webhook      (무인증 — 토스가 우리 Bearer 토큰을 가질 수 없다)
요청:  {"eventType": "...", "createdAt": "...", "data": {"orderId": "...", ...}}
응답:  {"received": true}
```

토스는 **200이 아니면 최대 7회, 3일 19시간에 걸쳐 재전송**한다(1분→4분→16분→…→4096분). 이걸 이용한다.

| 상황 | 코드 | 이유 |
|---|:---:|---|
| 처리했다 / 처리할 것이 없다(모르는 주문·모르는 이벤트·`orderId` 없음) | **200** | 재전송해도 결과가 같다 |
| 토스가 그 주문을 모른다 (`404 NOT_FOUND_PAYMENT`) | **200** | 〃 — 답이 달라질 수 없는 실패다 |
| 토스 조회 실패(통신·결제사 오류) | **502** | **아직 판단하지 못했다.** 흘리면 원장이 영영 어긋난다 |
| 토스 키 미설정 | **503** | 〃 (장애가 아니라 미구성) |

**실패를 두 갈래로 나누는 것이 이 표의 핵심이다.** "재시도하면 달라질 수 있는 실패"(통신·장애)만 재전송을 받고, "몇 번을 물어도 같은 답"(404)은 200으로 닫는다. 404를 502로 두면 무의미한 재전송을 7번 받는다 — 2026-07-26 실측에서 실제로 이 경로를 밟고 고쳤다. 이때 원장은 **건드리지 않는다**: 토스가 모른다는 것은 "승인된 적 없다"는 뜻이지만, 그 판정을 웹훅 경로에서 확정할 근거는 아니다.

요청 스키마(`TossWebhookEvent`)는 **모든 필드가 선택값**이다. 모르는 모양이 와도 422로 거절하지 않는다 — 거절하면 같은 실패를 7번 더 받을 뿐이다.

응답에 **처리 결과를 싣지 않는다.** 존재하지 않는 주문과 방금 취소된 주문의 응답이 구별되면, 무인증 호출자가 주문번호의 존재 여부를 떠볼 수 있다(다른 라우트의 404 존재 은닉과 같은 규칙).

### 동기화 규칙 (`billing_service.sync_payment_from_toss`)

| 토스 status | 원장 | 구독 |
|---|---|---|
| `CANCELED` · `PARTIAL_CANCELED` | `canceled` + `canceled_at`·`refunded_amount`·`refund_reason` | **건드리지 않는다** |
| `DONE` (원장이 done이 아닐 때) | `done` 승격 + `payment_key`·`method`·`approved_at` | 아직 반영 전이면 **활성화** |
| `ABORTED` · `EXPIRED` | `failed` + `fail_code` = 토스 status | — |
| `READY` · `IN_PROGRESS` · `WAITING_FOR_DEPOSIT` | 그대로 | — |
...
- **`BILLING_DELETED`(빌링키 삭제)** — 빌링키를 AES-GCM 암호문으로 저장해 평문 역조회가 안 된다(해시 컬럼을 새로 두면 가능하지만 마이그레이션이 붙는다). 그리고 삭제된 키는 **다음 갱신에서 청구 실패 → `past_due` → 재시도 창 만료**로 스스로 드러난다. 즉 방치해도 자연 복구되는 종류라, 화이트리스트에서 뺐다.
- **가상계좌·간편결제 이벤트** — 자동결제(카드 빌링) 단일 수단이라 해당 없다.

### 운영 설정 (코드로 되지 않는 것)

토스 **상점관리자 → 개발자센터 → 웹훅**에서 URL을 등록해야 알림이 온다. 등록하지 않으면 이 라우트는 영원히 호출되지 않는다.

```
https://api.kcalai.link/api/billing/webhook
이벤트: PAYMENT_STATUS_CHANGED · CANCEL_STATUS_CHANGED
```

> ⚠️ 토스는 **10초 이내 200**을 기대하는데 우리 조회 타임아웃은 `TOSS_TIMEOUT_SECONDS`(기본 10)다. 초과하면 토스가 재전송하고 그때 반영되므로 유실은 없지만, 타임아웃을 늘릴 때는 이 상한을 넘기지 않는 편이 낫다.

### 테스트

`tests/test_billing_webhook.py` **21건**. 토스는 호출하지 않는다(`get_payment_by_order_id`를 monkeypatch).

핵심은 `test_forged_cancel_does_not_touch_ledger` — 본문이 "취소"라고 해도 조회가 `DONE`이면 원장이 그대로다. 사실 이 속성은 시그니처가 이미 보장한다(`sync_payment_from_toss(db, order_id)`는 본문의 상태를 **받을 수 없다**). 커버: 모르는 주문에 토스 미호출 · 취소 반영 · 부분 취소 잔액 기준 · 재전송 멱등 · 취소가 구독을 회수하지 않음 · `failed`→`done` 복구 + 구독 활성화 · 활성 구독 기간 미연장 · 익명 원장 · `ABORTED`/`EXPIRED` 마감 · `done`을 뒤집지 못함 · 진행 중 무시 · 404는 200으로 닫힘 · 라우트 200/502/503 · 모르는 본문이 422가 아님 · 응답이 결과를 감춤.

### 실측 (2026-07-26, 로컬)

`venv/bin/python -m pytest` **333건 통과**(기존 312 + 21). `import main` OK, openapi 라우트 **50개**(`/api/billing/webhook` 추가, security 없음 = 무인증).

**방어 유효성은 되돌려 확인했다** — 원장 확인보다 토스 조회를 먼저 하도록 순서를 뒤집으면 2건이 실패한다(`test_unknown_order_does_not_call_toss`·`test_webhook_response_hides_result`).

curl 실측(별도 포트 8099, 로컬 `.env`에 토스 **테스트 키**가 있는 상태):

| 보낸 것 | 결과 |
|---|---|
| 모르는 주문 · 모르는 이벤트 · `orderId` 없음 · `{"hello":"world"}` · `{}` | 전부 **200** `{"received":true}`, 토스 미호출 |
| 무토큰 (대조: `POST /api/billing/cancel`) | 웹훅 **200** vs cancel **401** — 무인증 라우트 확인 |
| 원장에 있는 주문(토스엔 없음) | 조회가 실제로 나가 **404 `NOT_FOUND_PAYMENT`** → **200**으로 닫힘, 원장 `ready` 불변 |

이 실측이 세 가지를 동시에 확인해 준다: **Basic 인증 규격이 맞다**(인증이 통과해 404까지 갔다), **로그에 시크릿 키가 남지 않는다**(남은 것은 `code=NOT_FOUND_PAYMENT`뿐), **위조 본문이 원장을 바꾸지 못한다**(취소를 주장했지만 `status=ready` 그대로).
...
> ### ⚠️ 2026-08-03 결정 변경 — **A안 → C안 (IAP 단독)**
>
> **토스 자동결제(빌링) 계약에 초기비용 약 33만원이 든다는 것을 확인**하고(사용자 확인,
> 정확한 항목·조건은 토스 1544-7772 문의 필요), **웹 결제를 접고 IAP만 간다**로 바꿨다.
>
> **손익 계산이 명확하다.** Pro 5,000원 기준 건당 수령액은 토스(수수료 ~3%) 약 4,850원,
> IAP 15%(Apple 소규모 사업자 프로그램 · Google Play 연매출 100만 달러 이하) 4,250원 —
> 차이 약 600원이다. **33만원을 회수하려면 유료 구독 550건**, 즉 46명이 1년을 유지해야 한다.
> **현재 유료 사용자 0명**인 상태에서 낼 선투자가 아니다.
>
> **심사 관점에서는 오히려 단순해진다.** A안이 웹 토스를 유지한 것은 3.1.3(b)를 만족시키기
> 위해서였는데, **웹 결제 자체가 없어지면 그 조항이 무관해진다.** 남는 일은
> `toss-sdk.ts`의 "결제는 웹에서 진행해주세요" 문구를 지우는 것뿐이다.
>
> **버리지 않는 것** — 토스 코드는 **지우지 않는다**:
> - `payments` 원장은 전자상거래법상 **5년 보존 의무**가 있다
> - `scripts/refund_payment.py`·환불 웹훅(29장)은 **기존 결제의 환불 이행 수단**이다
> - `charge_due_subscriptions` cron은 유지하되, 아래 `provider='toss'` 조건이 있으면 대상 0건으로 안전하게 돈다
>
> 바꾸는 것은 **신규 결제 경로를 닫는 것**뿐이다 — `POST /api/billing/checkout`·`confirm`을
> 비활성화하고 웹 요금제 화면에서 구독 버튼을 내린다.
>
> **웹의 위치가 바뀐다.** 웹은 이제 **무료 이용·기록 조회·진료 리포트 인쇄** 용도이고
> 결제는 앱에서만 한다. ⚠️ 스토어 심사를 통과할 때까지 **유료 결제 경로가 0**이라는 뜻이며,
> 이는 감수한 트레이드오프다(현재 유료 사용자 0명).
>
> **2026-08-03 추가 결정 — 첫 출시는 결제 없이 낸다.**
>
> Play Billing 8 마감(8/31)은 **Billing Library 를 쓰는 앱**에만 걸린다. IAP 를 아직 넣지
> 않았고 웹 결제도 접었으므로, **결제 없는 무료 앱으로 먼저 출시하면 이 마감과 무관하다.**
> 나중에 IAP 를 붙일 때 그때의 최신 Billing 을 쓰면 된다. (연장 신청은 Play Console 을 통해
> 하는 것이라 개발자 계정이 없으면 신청할 방법도 없다 — 지금은 신청 대상이 아니다.)
>
> 순서상으로도 이쪽이 낫다: 의료 앱 심사(1.4.1)와 결제 심사를 **동시에** 통과하려 하면
> 리젝 사유가 두 배가 되고 원인 분리도 어렵다. 그리고 지금 가장 부족한 것은 결제가 아니라
> **외부 사용자 검증**이다(`PRODUCT_STRATEGY.md` §8 — "실제 환자 5명이 2주를 버티는지").
> 유료 사용자가 0명이라 잃는 것도 없다.
>
> **정리한 것 (2026-08-03)**: 앱에서 다른 결제수단을 가리키는 문구를 전부 제거했다 —
> `app/plan.tsx`·`app/billing/fail.tsx`의 "결제는 웹에서 진행해주세요" → "앱 내 구독은
> 준비 중이에요", `services/toss-sdk.ts`의 내부 방어 메시지도 같은 이유로 교체.
> 3.1.3 이 금지하는 것은 앱 안에서 **IAP 아닌 결제수단을 권유**하는 행위이므로,
> "준비 중"이라는 사실만 남기고 어디서 살 수 있는지는 말하지 않는다.

> **⏰ IAP 를 넣는 시점에는 마감이 걸린다** — 아래 'Google Play Billing 8' 절.
> 그리고 그 앞에 **더 급한 선행 조건**이 있다: `eas.json`이 없다 = **스탠드얼론 빌드를 한 번도
> 한 적이 없다.** IAP 이전에 **네이티브 앱이 동작하는지부터** 확인해야 한다(앱 알려진 문제 9 —
> Expo Go에서 카카오 로그인이 되지 않아 실기기 검증이 미완료다).

### (구) 채택안 — A안 (웹=토스 유지 · 앱=IAP 추가)
...
2. ~~**가격** — 스토어 수수료를 흡수할 것인가, 앱 가격을 웹보다 높일 것인가~~ → **C안에서 해소.** 웹에 파는 가격이 없으므로 비교 대상이 사라졌다. `plans.price_krw`를 스토어 상품 가격과 맞추기만 하면 된다.
3. ~~**기존 웹 구독자 처리**~~ → **해당 없음** (유료 구독자 0명). 다만 향후 생기면 3.1.3(b)로 유지 가능.
4. **Google Play의 자체 결제 옵션** — 2026-06-30부터 미국·영국·EEA에서 외부 결제가 열렸으나 **한국은 대상이 아니다.** 재확인 후 판단.
5. **소규모 사업자 프로그램 신청** (C안 추가) — Apple **App Store Small Business Program**은 **신청해야** 15%가 적용된다(자동 아님). Google Play는 연매출 100만 달러까지 15%가 자동 적용. **신청을 빠뜨리면 30%를 낸다** — 위 손익 계산이 무너진다.

### C안 실행 순서 (2026-08-03) — IAP보다 먼저 할 것이 있다

| # | 작업 | 왜 이 순서인가 |
|:-:|---|---|
| 1 | **개발자 계정 등록** (Apple $99/년 · Google $25 1회) | 사업자 정보가 필요했는데 이제 있다. 스토어 상품 등록의 전제 |
| 2 | **EAS Build 설정 + 실기기 스탠드얼론 빌드** | `eas.json`이 **없다**. 빌드가 되는지부터 모른다 |
| 3 | **네이티브 카카오 로그인 검증** | 앱 알려진 문제 9 — Expo Go는 스킴이 `exp://`라 동작하지 않아 **한 번도 확인되지 않았다.** 로그인이 안 되면 IAP는 의미가 없다 |
| 4 | **결제 경로 전환** — 웹 구독 버튼 내림 + `checkout`·`confirm` 비활성 | 신규 토스 결제를 닫는다. 원장·환불 경로는 유지 |
| 5 | **`expo-iap` 연동 + 서버 검증** | 위 스키마 변경(`provider`·`external_id`)과 함께 |
| 6 | 스토어 상품 등록 · 심사 제출 | 심사 기간이 마감에 포함된다 |

**2·3번이 진짜 리스크다.** 웹으로만 개발해 온 앱이라 네이티브 경로가 검증된 적이 없다.
IAP는 그 위에 얹는 것이라, 2·3이 막히면 4~6은 시작도 못 한다.
...
## 31. 진료 일정 — 케어 루프의 시작과 끝 (2026-08-19 확정 — 리비전 0028 `care_visits`)

이 앱의 완결은 "예약"이 아니라 **진료와 진료 사이 한 바퀴**다(`CARE_LOOP.md` §1). 그런데
그 바퀴가 언제 시작해 언제 끝나는지를 앱이 몰랐다 — §9의 열린 결정 4번("3개월 주기를 앱이
알 수 있는가")이 그것이고, 이 리비전이 그 답이다.

진료일을 알면 세 가지가 생긴다: **리포트를 언제 뽑아야 하는지**, **오늘 기록해야 할 이유**,
그리고 기간을 무엇으로 끊어야 하는지.

### 계약

| 메서드 | 경로 | 요청 | 응답 |
|---|---|---|---|
| `GET` | `/api/me/next-visit` | — | `{ scheduled_on: date\|null, notice }` |
| `PUT` | `/api/me/next-visit` | `{ scheduled_on: date, outcome?: str }` | 같음 (400 = 범위 밖, **403 = 메모에 동의 필요**) |
| `DELETE` | `/api/me/next-visit` | — | 204 (지울 것이 없어도 204 — 멱등) |

**등록되지 않은 상태는 404가 아니라 `scheduled_on: null`이다.** 404로 두면 앱이 "없음"과
"실패"를 구분하려고 예외 처리를 하게 된다. 등록하지 않은 것은 오류가 아니라 정상 상태다.

**D-day를 서버가 계산하지 않는다.** 서버 시각은 UTC이고 사용자는 자기 지역의 '오늘'에서
남은 날을 센다 — 서버가 계산하면 자정 전후로 하루가 어긋난다. 날짜만 주고 앱이 센다.

### 진료 메모 (`outcome`, 2026-08-19 추가)

진료에서 들은 것(식단 지침·주의사항)을 사용자가 옮겨 적는 자유 텍스트다. 이것으로
`CARE_LOOP.md` §1 도식 좌측의 **"환자가 진료에서 받은 것을 앱에 이어 적는다"**가 성립한다.

- **`outcome` 생략 = 안 건드림, 빈 문자열 = 지움.** 날짜만 고치는 요청이 메모를 날리면 안 되고,
  잘못 적은 메모를 지울 방법도 있어야 한다.
- **동의는 필드 단위로 요구한다.** 날짜만 보내면 동의가 필요 없고, **메모에 내용이 있으면**
  `sensitive_health` 동의를 요구한다(403) — 자유 텍스트라 질병 정보가 들어올 수 있기 때문이다.
...
- **예정은 사용자당 하나.** 여러 예약을 관리하는 캘린더가 아니라 "다음 한 바퀴가 언제
  끝나는가"를 아는 값이라, 새로 넣으면 기존 예정을 덮어쓴다.
- **`sensitive_health` 동의를 요구하지 않는다.** 검사 수치(§4)와 다른 판단이고 근거는 이
  라우트가 다루는 값이 **날짜 하나**라는 점이다 — 질병명도 수치도 없다. 동의를 요구하면
  온보딩 직후 홈에 D-day를 못 그리는데, 그러면 이 기능의 존재 이유가 첫날부터 사라진다.
  가이드(`/api/guides`)를 동의 없이 여는 것과 같은 층위다.
  > ⚠️ `clinic_label`(병원 이름)을 API로 여는 순간 이 판단을 다시 한다 — "OO신장내과"는
  > 질환을 추론하게 하므로 그때는 민감정보에 가깝다.
- **범위 검증은 오타 방어일 뿐이다.** 과거 1년~미래 5년 밖이면 400. 최근 과거를 허용하는
  이유는, 진료를 다녀오고 다음 일정을 아직 못 정한 사람의 날짜가 지나 있는 것이 정상이기
  때문이다.
- **예약 시스템이 아니다.** 병원과 아무것도 주고받지 않는다. 방향이 반대라야 의료법
  제27조 제3항(영리 목적 소개·알선 금지)에 닿지 않는다(`CARE_LOOP.md` §3). 같은 이유로
  `clinic_label`은 자유 텍스트이고 병원 마스터 DB를 참조하지 않는다.

### 테이블

`care_visits` — 컬럼은 `CARE_LOOP.md` §4-3 설계 그대로 만들었고, **지금 API가 여는 것은
`scheduled_on` 하나**다. `questions`·`outcome`·`clinic_label`·`report_from`/`report_to`는
진료 질문·결과 기록을 붙일 때 그대로 쓴다 — 컬럼 몇 개의 비용보다 마이그레이션을 또 도는
비용이 크다.
