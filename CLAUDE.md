...
>
> ⚠️ **운영 서버에서는 4번(backfill)을 직접 돌릴 수 없습니다** — 원본 공공 CSV 를 서버에 두지 않기 때문입니다. 반드시 **로컬에서 `--emit-sql <경로>`로 SQL 을 뽑아 운영에 적용**하세요. 2026-07-25에 이 단계를 빠뜨려 운영 curated 100행의 나트륨·칼륨이 전부 비었고, 국·탕·찌개·과일에서 CKD·고혈압 경고가 침묵했습니다(2026-07-22 사건의 재발). 지금은 원본을 못 찾으면 스크립트가 **중단**하지만, 절차 자체를 기억하는 편이 낫습니다.
...
| Consent | `GET·POST /api/me/consents` · `POST /api/me/consents/revoke` · `GET·PUT /api/me/health-profile` (**2026-07-23: `ckd_stage` 추가** — 신장병 병기, 비암호화 기능 키. ⚠️ 이 PUT은 **전체 교체**라 병기만 바꿀 때도 혈액형·Rh를 함께 보낸다) · `GET·PUT /api/me/conditions` · `GET·PUT /api/me/allergies` | `api/consent_api.py` |
...
| Meta | `GET /api/meta/options` (**2026-07-23: `ckd_stages` 추가** — 병기 3종. 지침에서 온 고정값이지만 라벨은 서버가 정한다) | `api/meta_api.py` |
...
- **민감정보를 담는 테이블을 추가하면 `consent_service._destroy_sensitive_data`에도 반영한다.** 탈퇴 연쇄(`account_service.delete_account`)와 **서로 다른 목록**이라 한쪽만 고치면 동의를 철회해도 데이터가 남습니다 — 2026-08-19에 `lab_results`(리비전 0027)가 그 상태였습니다. 조회가 403으로 막히는 것과 파기는 다릅니다. 그리고 **앱의 철회 확인 문구**(`k-calAI-RN`의 `app/me/consents.tsx`)를 같은 작업 단위에서 고치세요 — 무엇을 잃는지 모르고 누르면 고지가 아닙니다. 규약은 `tests/test_consent_revoke_destroys.py`.
...
| **출시 전 법적 요건 (연령·환불·프라이버시 매니페스트·스토어 심사)** | **`docs/LEGAL_COMPLIANCE.md`** (2026-07-25 조사, 07-26 §6 스토어 요건 추가. ⚠️ 법률 자문이 아니며 출시 전 전문가 검토 필요. 만 14세 미만 차단은 **형사처벌이 걸린 항목**이라 근거를 읽고 손댈 것) |
...
| **경쟁 지형 · 이 기능이 이미 있는가** | **`docs/COMPETITIVE_LANDSCAPE.md`** (2026-08-01 조사. ⚠️ **대한신장학회 공식 앱 '하이디'가 검사수치·교육자료를 이미 갖고 있다**(2020-04 업데이트 정지). 새 기능을 "우리 아이디어"라고 부르기 전에 읽는다. 빈자리는 **병존**뿐) |
| **워크플로우 · 어디서 끊겨 있나 · 병원 연계 경계** | **`docs/CARE_LOOP.md`** (2026-08-01 기획: 완결은 "예약"이 아니라 "진료와 진료 사이 한 바퀴". 검사 수치·질환 가이드·진료 탭. ⚠️ **병원 예약 중개는 하지 않는다** — 의료법 제27조 제3항·비대면진료 플랫폼 신고 의무, §3) |
