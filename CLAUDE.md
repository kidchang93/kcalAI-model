...
| Health | `GET·PUT /api/me/profile` · `GET·PUT /api/me/goal` · `GET /api/me/summary` (**2026-07-23: `nutrients` 추가** — 질환 축 하루 누적. 나트륨만 `limit_mg`(상한 대비 게이지), 칼륨·인은 수치와 투석 참고치만. 해당 질환 없으면 null. `docs/DATA_MODEL.md` 28장) · `GET /api/me/trends` · `POST·GET /api/meals` · `PUT·DELETE /api/meals/{meal_id}` · `POST·GET /api/weights` | `api/health_api.py` |
