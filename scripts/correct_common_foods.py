...
CORRECTIONS: list[tuple[str, int, str]] = [
    ...
    ("미역국", 80, "1인분 (약 300g)"),
    ("콩나물국", 40, "1인분 (약 300g)"),
    # 북엇국·소바(2026-08-03): 1인분 무게가 아예 없어(`serving_size_g` NULL) backfill 이
    # 환산을 못 해 별칭을 찾고도 영양소를 채우지 못했다. 값은 **추정이 아니라 원본의
    # 식품중량**이다 — 식약처 CSV 의 `북어국` 400 g(28 kcal/100g) · `메밀소바` 400 ml
    # (86 kcal/100ml). 그래서 kcal 도 그 기준으로 환산한 값이다(112·344).
    ("북엇국", 112, "1인분 (약 400g)"),
    ("소바", 344, "1인분 (약 400g)"),
    ("볶음밥", 600, "1인분 (약 350g)"),
    ("소불고기", 400, "1인분 (약 200g)"),
    ...
    ("계란찜", 160, "1인분 (약 200g)"),
    ("족발", 480, "1인분 (약 200g)"),
    # 보쌈·수육: DB에 고기 요리로 없어 "보쌈" 검색이 보쌈김치(33kcal)로 오매칭되던 것 보강.
    ("보쌈", 470, "1인분 (약 200g)"),
    ("수육", 400, "1인분 (약 200g)"),
    ("돼지고기수육", 400, "1인분 (약 200g)"),
...
#
# ⚠️ **이 스크립트를 실행한 뒤에는 반드시 `backfill_curated_nutrients.py` 를 이어서 돌린다.**
# 여기서 영양소를 비우면 그 음식은 경고 판정에서 조용히 빠진다 — 2026-07-22 에 실제로
# 된장찌개·설렁탕·미역국 등 국·탕·찌개와 과일 28건의 나트륨·칼륨·인이 사라져,
# 고혈압 나트륨 등급은 물론 **이미 운영 중이던 CKD 칼륨·인 경고까지 무력했다.**
# 비우는 것 자체는 옳다(옛 서빙 기준 값이 새 1인분과 불일치). 비운 뒤 되채우는 것이 규약이다.
_NULLED = (
    "carbs_g",
    "protein_g",
    "fat_g",
    ...
    rows = [
        {
            ...
            # serving_desc("1인분 (약 200g)")에서 1인분 무게를 뽑는다 (ml 은 밀도≈1). 못 뽑으면 None.
